...
class ApiService
{
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public static function advs($advId)
    {
        $advCount = Db::name('adv')->where('id', $advId)->where(['status' => 1, 'delete_time' => 0])->count();

        if ($advCount == 0) {
            return [];
        }

        $advs = Db::name('adv_item')->where('status', 1)->where('adv_id', $advId)->order('list_order ASC')->select();
