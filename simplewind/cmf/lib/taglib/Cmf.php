...
class Cmf extends TagLib
{
    ...
    protected $tags = [
        ...
        'advs'              => ['attr' => 'id', 'close' => 1],//非必须属性item
        'noadvs'            => ['attr' => 'id', 'close' => 1],
...
parse;
     ...
     * 幻灯片标签
     */
    public function tagAdvs($tag, $content)
    {
        $id    = empty($tag['id']) ? '0' : $tag['id'];
        $item  = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $parse = <<<parse
<?php
     \$__ADV_ITEMS__ = \app\admin\service\ApiService::advs({$id});
?>
<volist name="__ADV_ITEMS__" id="{$item}">
{$content}
</volist>
parse;
     ...
     * 无幻灯片标签
     */
    public function tagNoAdvs($tag, $content)
    {
        $id    = empty($tag['id']) ? '0' : $tag['id'];
        $parse = <<<parse
<?php
    if(!isset(\$__ADV_ITEMS__)){
        \$__ADV_ITEMS__ = \app\admin\service\ApiService::advs({$id});
    }
?>
<if condition="count(\$__ADV_ITEMS__) eq 0">
{$content}
</if>
parse;
